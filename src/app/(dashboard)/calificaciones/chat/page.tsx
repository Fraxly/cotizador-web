...
const GREETING: Message = {
  role: "assistant",
  content: "¡Hola! Te ayudo a armar el documento de **calificaciones, proyectos comparables y términos** para acompañar una propuesta.\n\nAdjunta el TDR o cuéntame para qué cliente/licitación es.",
}

function SummaryPanel({ state }: { state: QualificationsState }) {
  if (!state.cliente && state.casos.length === 0) return null

  return (
    <div className="bg-[#f5f5f7] rounded-2xl p-5 space-y-4 text-sm">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-[#1d1d1f]">Documento</h3>
        <span className="text-[#86868b] text-xs">
          {state.completo ? "✅ Listo" : "🔄 En progreso"}
        </span>
      </div>
      ...
      {state.casos.length > 0 && (
        <div>
          <span className="text-[#86868b] text-xs">Casos incluidos ({state.casos.length})</span>
          <div className="mt-1.5 space-y-1">
            {state.casos.map((c, i) => (
              <div key={i} className="text-[#1d1d1f]">{c.cliente} — {c.servicio_titulo}</div>
            ))}
          </div>
        </div>
      )}

      {state.terminos.length > 0 && (
        <div>
          <span className="text-[#86868b] text-xs">Términos ({state.terminos.length})</span>
        </div>
      )}
    </div>
  )
...
export default function CalificacionesChatPage() {
  return (
    <Suspense>
      <CalificacionesChatPageInner />
    </Suspense>
  )
}

function CalificacionesChatPageInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [messages, setMessages] = useState<Message[]>([GREETING])
  const [input, setInput] = useState("")
  const [loading, setLoading] = useState(false)
  const [docState, setDocState] = useState<QualificationsState | null>(null)
  ...
  useEffect(() => {
    ...
    if (cId) {
      fetch(`/api/conversations/${cId}`)
        ...
        .then(data => {
          if (data) {
            ...
            if (data.state) setDocState(data.state)
      ...
      fetch("/api/conversations", {
        ...
        body: JSON.stringify({ tipo: "qualifications", messages: [GREETING], state: null }),
      })
        .then(r => r.json())
        .then(data => {
          setConversationId(data.id)
          router.replace(`/calificaciones/chat?c=${data.id}`)
        })
    }
  ...
  const sendContent = useCallback(async (content: string, displayContent?: string) => {
    ...
    try {
      const res = await fetch("/api/chat/qualifications", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const newState = data.state || docState
      if (data.state) setDocState(data.state)
      ...
      if (conversationId) {
        fetch(`/api/conversations/${conversationId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ messages: afterError, state: docState }),
        }).catch(() => {})
      }
    } finally {
      setLoading(false)
    }
  }, [loading, messages, conversationId, docState])
  ...
  async function handleGeneratePdf() {
    if (!docState || docState.casos.length === 0) return
    ...
    try {
      const res = await fetch("/api/qualifications", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          cliente: docState.cliente || "Cliente",
          subtitulo: docState.subtitulo,
          idioma: docState.idioma || "es",
          about_text: docState.about_text,
          casos: docState.casos,
          terminos: docState.terminos,
        }),
      })
      ...
      window.open(`/api/qualifications/${id}/pdf`, "_blank")
  ...
  const canGeneratePdf = docState?.completo && docState.casos.length > 0
    ...
    <div className="h-[calc(100vh-56px)] flex">
      ...
      <div className="w-80 border-l border-[#e8e8ed] bg-white p-5 hidden lg:block overflow-y-auto">
        <div className="space-y-5">
          <SummaryPanel state={docState || { cliente: "", subtitulo: "", idioma: "es", about_text: "", casos: [], terminos: [], completo: false }} />
          ...
          {docState && !docState.completo && docState.casos.length > 0 && (
